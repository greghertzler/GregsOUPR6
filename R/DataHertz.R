...
#'  say z(0), a stationary increment is added to give the next state, z(1).  That state becomes
#'  the initial condition of the next observation to which another stationary increment is added
#'  to give the next state, z(2).  This can be solved to show that z(2)=z(0)+e(0)+e(1).
#'  Continuing on, Brownian Motion at any time is the initial fixed state plus a running total
#'  of the stationary increments.
#'
#' The test for Brownian Motion is the opposite of the test for stationary increments.  Parameter
#'  rho is expected to be zero.  As rho goes to zero, the mean of the Ornstein-Uhlenbeck
#'  Process goes to z(s).  In other words, the expected value of the next state equals the
#'  observed state at the beginning of each observation.  The variance goes to sigma^2(t-s),
#'  which equals sigma^2 in this date with time intervals equal to one.
#'
#' To conduct a test, first estimate the unrestricted parameters.  Then impose a restriction.
#'  Set rho=0. There is no need to restrict mu because it cancels from all formulas and the
#'  estimation algorithm will set it to zero for you.
#'
...
#'  sigma should not be significantly different from one.
#'
#' @docType data
#' @keywords datasets
...
#'
#' Stationary increments are created from pseudo-random standard normal variables generated by
...
#'  time intervals equal to one and eps from dqrng, an optional package, mt19937, the generator
#'  in c++, sitmo, an optional package, and rnorm, the default generator in R.
#'
...
#'
#' To conduct a test, first estimate the unrestricted parameters.  With time intervals equal
#'  to one, parameter rho equal to 3 or more is closer to infinity than to zero.  Then impose
#'  restrictions. Choose a large number with an easy square root, such as 1600.  Set 2rho=1600,
#'  or rho=800. Set mu=0.  Set sigma=40, which is the square root of 1600 and imposes the
