...
#' \itemize{
#'   \item tau: time variable
#'   \item e(s): stationary increments e(s)=eps(t-s)^0.5 where eps is a pseudo-random standard normal variable
#'   \item z(t)=z(s)+e(s): Brownian Motion from stationary increments
#'   \item z(t)-z(s)=e(s): residuals of Brownian Motion equal to the increments
#' }
#'
#' Brownian Motion, also called a Wiener Process, does not converge and can never be stationary.
#'  Paradoxically, it is constructed from stationary increments.  Beginning from a fixed state,
#'  say z(0), a stationary increment is added to give the next state, z(1)=z(0)+e(0).  This is
#'  the simplest stochastic integral equation with mean z(0) and variance e(0)^2.  State z(1) becomes
#'  the initial condition of the next observation to which another stationary increment is added
#'  to give the next state, z(2)=z(1)+e(1), and so on.
#'
#' The stochastic integral equation can be defined over any time interval.  Suppose z(1) is
#'  substituted into z(2) to show that z(2)=z(0)+e(0)+e(1). The mean is z(0), but the variance
#'  is e(0)^2+e(1)^2.  Continuing on, Brownian Motion at any time is the initial fixed state plus
#'  a running total of the stationary increments.  The longer the time interval, the larger the
#'  variance.
#'
#' The test for Brownian Motion is the opposite of the test for stationary increments.  Parameter
#'  rho is expected to be zero.  If rho is zero, the mean of the Ornstein-Uhlenbeck Process
#'  is z(s).  The expected value of the next state equals the observed state at the beginning
#'  of each observation.  The variance of the Ornstein-Uhlenbeck Process goes to sigma^2(t-s).
#'
#' To conduct an hypothesis test, first estimate the unrestricted parameters.  Then impose a
#'  restriction. Set rho=0. There is no need to restrict mu because it cancels from all
#'  formulas and the estimation algorithm will set it to zero for you.
#'
#' Then do a Likelihood Ratio Test comparing the unrestricted and restricted estimates.  If
#'  the Log Likelihoods are not significantly different, the hypothesis of Brownian Motion
#'  cannot be rejected.
#'
#' Because the stationary increments are eps(t-s)^0.5, where eps~N(0,1) and t-s=1, parameter
#'  sigma should not be significantly different from one.  For Scaled Brownian Motion, sigma
#'  will be some other constant.
#'
#' @docType data
#' @keywords datasets
#' @name OUP_BrownianMotion
#' @format csv file with 1001 rows and 4 columns
NULL
...
#'
#' Stationary increments are created from pseudo-random standard normal variables.
#'
#' \itemize{
#'   \item tau: time variable
#'   \item dqrng: dqrng::pcg64 uniform random numbers with Ziggurat transform to normal
#'   \item mt19937: std::mt19937 uniform random numbers with Polar Box-Muller transform to normal
#'   \item sitmo:  sitmo::prng uniform random numbers with Polar Box-Muller transform to normal
#'   \item rnorm:  rnorm uniform random numbers with cumulative normal inversion
#' }
#'
#' An important assumption of the Ornstein-Uhlenbeck Process is stationary increments. An
#'  increment is eps(t-s)^0.5, where eps~N(0,1) and t-s is a time interval.  This data has
#'  time intervals equal to one and eps from random number generators dqrng, mt19937, sitmo
#'  and rnorm.
#'
#' Random number generators must pass many tests.  Here we test for stationary increments as
#'  a special case of the Ornstein-Uhlenbeck Process.  The Ornstein-Uhlenbeck Process is
#'  stationary if rho goes to infinity and sigma goes to infinity in such a way that
#'  sigma/(2rho)^0.5 goes to a constant.
#'
#' To conduct an hypothesis test, first estimate the unrestricted parameters.  With time
#'  intervals equal to one, an estimated rho equal to 3 or more is closer to infinity than
#'  to zero.  Then impose restrictions. Choose a large number with an easy square root,
#'  such as 1600.  Set 2rho=1600, or rho=800. Set mu=0.  Set sigma=40, which is the square
#'  root of 1600 and imposes the restriction that sigma/(2rho)^0.5=1.
#'
#' Then do a Likelihood Ratio Test comparing the unrestricted and restricted estimates.  If
#'  the Log Likelihoods are not significantly different, the hypothesis of stationary increments
#'  cannot be rejected.
#'
#' @docType data
#' @keywords datasets
#' @name OUP_StationaryIncrements
#' @format csv file with 1001 rows and 5 columns
NULL
