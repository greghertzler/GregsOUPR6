...
convenience, every calculation and plot function can copy tab-delimited
text to the clipboard. The text can be pasted as a table in most other
applications. Copying is off by default but you turn it on from any
...
automatically call plot routines is also set to TRUE.
